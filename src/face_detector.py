    ...
    def _init_mediapipe(self):
      ...
      print(" MediaPipe FaceLandmarker Task Engine fully initialized.")
       """
        ...
        # ── Face Mesh ─────────────────────────────────────────────
        # 468 landmarks for fine-grained expression analysis
        self.mp_face_mesh = mp.solutions.face_mesh
        self.face_mesh = self.mp_face_mesh.FaceMesh(
            static_image_mode=True,         # single images, not video
            max_num_faces=1,
            refine_landmarks=True,          # enables iris landmarks (477 total)
            min_detection_confidence=self.config.MESH_DETECTION_CONF,
            min_tracking_confidence=self.config.MESH_TRACKING_CONF
        )
       ...
       # Run MediaPipe face detection.
        ...
        if len(results.detections) > 1:
            return {
                ...
                "message" : f"{len(results.detections)} faces detected. "
        ...
        x = max(0, int(bbox_norm.xmin  * w))
        y = max(0, int(bbox_norm.ymin  * h))
        bw = min(int(bbox_norm.width   * w), w - x)
        bh = min(int(bbox_norm.height  * h), h - y)
...
"""
    ...
    def _check_expression(self, landmarks, img_w: int, img_h: int) -> dict:
        ...
        face_height = np.linalg.norm(px(Landmarks.FOREHEAD_TOP) -
                                     px(Landmarks.CHIN_BOTTOM))
        face_width  = np.linalg.norm(px(Landmarks.FACE_LEFT) -
                                     px(Landmarks.FACE_RIGHT))
        ...
        mouth_open = np.linalg.norm(
            px(Landmarks.UPPER_LIP_TOP) - px(Landmarks.LOWER_LIP_BOTTOM)
        ) / face_height
        ...
        mouth_width = np.linalg.norm(
            px(Landmarks.MOUTH_LEFT) - px(Landmarks.MOUTH_RIGHT)
        )
        ...
        # ─────────────────────────────────────────────────────────
        right_brow_raise = np.linalg.norm(
            px(Landmarks.RIGHT_BROW_CENTER) - px(Landmarks.RIGHT_EYE_TOP)
        ) / face_height

        left_brow_raise  = np.linalg.norm(
            px(Landmarks.LEFT_BROW_CENTER) - px(Landmarks.LEFT_EYE_TOP)
        ) / face_height
        ...
        if mouth_open > cfg.MOUTH_OPEN_THRESHOLD:
            reasons.append(
                f"mouth open (score {scores['mouth_open']:.3f} > "
                f"threshold {cfg.MOUTH_OPEN_THRESHOLD})"
            )

        if mouth_stretch > cfg.MOUTH_STRETCH_THRESHOLD:
            reasons.append(
                f"wide smile (score {scores['mouth_stretch']:.3f} > "
                f"threshold {cfg.MOUTH_STRETCH_THRESHOLD})"
            )

        if brow_raise > cfg.EYEBROW_RAISE_THRESHOLD:
            reasons.append(
                f"raised eyebrows (score {scores['brow_raise']:.3f} > "
                f"threshold {cfg.EYEBROW_RAISE_THRESHOLD})"
            )

        if min_ear < cfg.EYE_SQUEEZE_THRESHOLD:
            reasons.append(
                f"eye squint/wink (EAR {scores['min_eye_EAR']:.3f} < "
                f"threshold {cfg.EYE_SQUEEZE_THRESHOLD})"
            )

        if tilt_deg > cfg.HEAD_TILT_THRESHOLD_DEG:
            reasons.append(
                f"head tilt {scores['head_tilt_deg']}° > "
                f"threshold {cfg.HEAD_TILT_THRESHOLD_DEG}°"
            )
    ...
    def _draw_detection(self, image: np.ndarray, bbox, confidence) -> np.ndarray:
        ...
        cv2.putText(image, label, (x, max(y-8, 12)),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
        return image

    def _draw_debug(self, image: np.ndarray, bbox, confidence,
                    landmarks, img_w, img_h) -> np.ndarray:
    ...
    def _fail(self, status: ValidationStatus, message: str,
              expression_scores=None) -> FaceValidationResult:
        return FaceValidationResult(
            status=status,
            is_valid=False,
            message=message,
